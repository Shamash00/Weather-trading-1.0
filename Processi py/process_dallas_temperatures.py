...
# Load all CSV files starting with 72258013960
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "72258013960*.csv")))
...
# Parse TMP: format is "+0211,5" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Dallas local time (CST=UTC-6, CDT=UTC-5)
dallas_tz = ZoneInfo("America/Chicago")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(dallas_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Dallas", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Dallas").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Dallas"] = output["Data_Ora_Locale_Dallas"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Dallas Orario Locale 2021-2025.xlsx")
