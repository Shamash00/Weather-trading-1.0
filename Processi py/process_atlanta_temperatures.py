...
# Load all CSV files starting with 72219013874
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "72219013874*.csv")))
...
# Parse TMP: format is "+0150,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Atlanta local time (EST=UTC-5, EDT=UTC-4)
atlanta_tz = ZoneInfo("America/New_York")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(atlanta_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Atlanta", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Atlanta").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Atlanta"] = output["Data_Ora_Locale_Atlanta"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Atlanta Orario Locale 2021-2025.xlsx")
