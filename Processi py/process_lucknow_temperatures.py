...
# Load all CSV files starting with 42369099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "42369099999*.csv")))
...
# Parse TMP: format is "+0036,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Lucknow local time (IST = UTC+5:30, no DST)
lucknow_tz = ZoneInfo("Asia/Kolkata")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(lucknow_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Lucknow", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Lucknow").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Lucknow"] = output["Data_Ora_Locale_Lucknow"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Lucknow Orario Locale 2021-2025.xlsx")
