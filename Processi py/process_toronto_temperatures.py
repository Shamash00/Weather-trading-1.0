...
xl = pd.ExcelFile("Temperature Registrate Aereoporto Toronto 2021-2025.xlsx")
...
# Convert DATE from UTC to Toronto local time (handles EST/EDT automatically)
toronto_tz = ZoneInfo("America/Toronto")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(toronto_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Toronto", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Toronto").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Toronto"] = output["Data_Ora_Locale_Toronto"].dt.tz_localize(None)
...
# Save to Excel
out_path = "Temperature FM-15 Toronto Orario Locale 2021-2025.xlsx"
