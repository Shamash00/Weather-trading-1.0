...
# Load all CSV files starting with 10866099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "10866099999*.csv")))
...
# Parse TMP: format is "+0040,5" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Munich local time (CET=UTC+1, CEST=UTC+2)
munich_tz = ZoneInfo("Europe/Berlin")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(munich_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Monaco", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Monaco").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Monaco"] = output["Data_Ora_Locale_Monaco"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Monaco Orario Locale 2021-2025.xlsx")
