...
# Load all CSV files starting with 47113199999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "47113199999*.csv")))
...
# Convert DATE from UTC to Seoul local time (KST = UTC+9, no DST)
seoul_tz = ZoneInfo("Asia/Seoul")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(seoul_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Seoul", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Seoul").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Seoul"] = output["Data_Ora_Locale_Seoul"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Seoul Orario Locale 2021-2025.xlsx")
