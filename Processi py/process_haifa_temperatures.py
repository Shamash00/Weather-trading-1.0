...
# Load all CSV files starting with 40155099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "40155099999*.csv")))
...
# Parse TMP: format is "+0082,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Haifa local time (IST=UTC+2, IDT=UTC+3)
haifa_tz = ZoneInfo("Asia/Jerusalem")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(haifa_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Haifa", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Haifa").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Haifa"] = output["Data_Ora_Locale_Haifa"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Haifa Orario Locale 2021-2025.xlsx")
