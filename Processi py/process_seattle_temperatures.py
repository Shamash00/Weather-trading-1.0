...
# Load all CSV files starting with 72793024233
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "72793024233*.csv")))
...
# Parse TMP: format is "+0083,5" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Seattle local time (PST=UTC-8, PDT=UTC-7)
seattle_tz = ZoneInfo("America/Los_Angeles")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(seattle_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Seattle", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Seattle").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Seattle"] = output["Data_Ora_Locale_Seattle"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Seattle Orario Locale 2021-2025.xlsx")
