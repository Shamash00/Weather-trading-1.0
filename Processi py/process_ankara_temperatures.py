...
# Load all CSV files starting with 17128099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "17128099999*.csv")))
...
# Parse TMP: format is "+0020,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Ankara local time (TRT = UTC+3, no DST since 2016)
ankara_tz = ZoneInfo("Europe/Istanbul")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(ankara_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Ankara", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Ankara").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Ankara"] = output["Data_Ora_Locale_Ankara"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Ankara Orario Locale 2021-2025.xlsx")
