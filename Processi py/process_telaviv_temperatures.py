...
# Load all CSV files starting with 40180099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "40180099999*.csv")))
...
# Parse TMP: format is "+0117,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Tel Aviv local time (IST=UTC+2, IDT=UTC+3)
telaviv_tz = ZoneInfo("Asia/Jerusalem")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(telaviv_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_TelAviv", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_TelAviv").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_TelAviv"] = output["Data_Ora_Locale_TelAviv"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 TelAviv Orario Locale 2021-2025.xlsx")
