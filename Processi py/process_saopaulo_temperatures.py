...
# Load all CSV files starting with 83075099999
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "83075099999*.csv")))
...
# Convert DATE from UTC to Sao Paulo local time (BRT = UTC-3, no DST since 2019)
saopaulo_tz = ZoneInfo("America/Sao_Paulo")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(saopaulo_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_SaoPaulo", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_SaoPaulo").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_SaoPaulo"] = output["Data_Ora_Locale_SaoPaulo"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 SaoPaulo Orario Locale 2021-2025.xlsx")
