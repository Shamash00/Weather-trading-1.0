...
# Load all CSV files starting with 72202012839
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "72202012839*.csv")))
...
# Parse TMP: format is "+0244,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Miami local time (EST=UTC-5, EDT=UTC-4)
miami_tz = ZoneInfo("America/New_York")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(miami_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Miami", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Miami").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Miami"] = output["Data_Ora_Locale_Miami"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Miami Orario Locale 2021-2025.xlsx")
