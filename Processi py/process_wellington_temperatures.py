...
# Load all CSV files starting with 93436000488
base_dir = os.path.dirname(os.path.abspath(__file__))
files = sorted(glob.glob(os.path.join(base_dir, "93436000488*.csv")))
...
# Parse TMP: format is "+0190,1" where value before comma = temp*10, after comma = quality flag
...
# Convert DATE from UTC to Wellington local time (NZST=UTC+12, NZDT=UTC+13)
wellington_tz = ZoneInfo("Pacific/Auckland")

df_fm15["DATE_UTC"] = pd.to_datetime(df_fm15["DATE"], utc=True)
df_fm15["DATE_LOCAL"] = df_fm15["DATE_UTC"].dt.tz_convert(wellington_tz)

# Build clean output
output = df_fm15[["DATE_LOCAL", "TMP_C"]].copy()
output.columns = ["Data_Ora_Locale_Wellington", "Temperatura_C"]
...
# Sort by date
output = output.sort_values("Data_Ora_Locale_Wellington").reset_index(drop=True)

# Remove timezone info for cleaner Excel output
output["Data_Ora_Locale_Wellington"] = output["Data_Ora_Locale_Wellington"].dt.tz_localize(None)
...
# Save to Excel
out_path = os.path.join(base_dir, "Temperature FM-15 Wellington Orario Locale 2021-2025.xlsx")
